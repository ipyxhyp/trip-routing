...
public class RoutingService {
    ...
    /**
     * Reads the countries.json resource file and populates the countryList and countryMap internal fields
     *
    ...
    public List<String> searchRoutingPath(final String sourceCountryCode, String targetCountryCode) throws Failure {
        ...
        while (!vertexCountryList.isEmpty() && !isMatchedTarget) {
            Country smallest = vertexCountryList.stream().reduce(targetCountry, (prevCountry, currCountry) ->
                (distanceMap.get(currCountry.getCode()) < distanceMap.get(prevCountry.getCode()) ? currCountry
                    : prevCountry));
            if (smallest != null) {
                ...
                if (!CollectionUtils.isEmpty(smallest.getBorders())) {
                    smallest.getBorders().forEach(
                        border -> {
        ...
        if (path.isEmpty() || (path.size() == 1 && path.get(0).equals(targetCountry.getCode()))) {
            LOGGER.error("No direct land borders between the countries : {} , {} ", sourceCountryCode,
                targetCountry.getCode());
            throw new Failure(String.format("No direct land borders between the countries : %s, %s ", sourceCountryCode,
                targetCountry.getCode()),
    ...
    private void validateCountryCodes(String sourceCountryCode, String targetCountryCode, Country targetCountry,
        ...
        if (targetCountry == null || sourceCountry == null) {
            LOGGER.error("Either targetCountry or sourceCountry does not exist by the provided country codes : {}, {} ",
                sourceCountryCode, targetCountryCode);
            throw new Failure(String
                .format("Either targetCountry or sourceCountry does not exist by the provided country codes : %s, %s ",
                    sourceCountryCode, targetCountryCode),
                HttpURLConnection.HTTP_BAD_REQUEST, ReasonType.CODE_400);
        }
        if (targetCountry != null && sourceCountry != null && targetCountry.equals(sourceCountry)) {
            LOGGER.error("Target Country is the same as the Origin Country : {}, {} ", sourceCountryCode,
                targetCountryCode);
            throw new Failure(String
                .format("Target Country is the same as the Origin Country : %s, %s ", sourceCountryCode,
                    targetCountryCode),
                HttpURLConnection.HTTP_BAD_REQUEST, ReasonType.CODE_400);
        }
    }
